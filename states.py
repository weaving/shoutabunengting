...
class SimulatedUniverse(BattleList):
    ...
    def start(self):  # gzz
        ...
        self.ref_abs_pos_x = 777
        self.ref_abs_pos_y = 440
        ...
        while True:  # 这才是大循环吧
            ...
            while True:  # 每个周期 计算游戏该干嘛
                self.sub_data_init()
                # 符文出现的时候 别干了
                if (self.run_time > 465 -  15 and  self.run_time < 465 + 5 ) or \
                    (self.run_time > 917 - 15 and self.run_time < 917 + 5) or \
                    (self.run_time > 1369 - 15 and self.run_time < 1369 + 5) :
                    if self.check('yincang', 0, 960, 540, self.screen_max_y):
                        timeout = 5
                        while timeout:
                            for list in self.qianghua_list:
                                if self.check_qianghua(list):
                                    self.click()
                                    self.checkAndClick("xuanze", 1000, 1300, 900, 1000)
                                    break
                            timeout = timeout - 1
                            time.sleep(1)
                            self.get_screen()
                ...
                if self.run_time > 360 : # 金币充能
                        self.jinbichongneng_time = self.run_time
                        self.jinbichongneng()

                if self.run_time > 180 and self.expect_flag == 0 and self.touxiang_en: #看看队友是不是充钱了
                    self.expect_result()
                    self.expect_flag = 1

                if self.start_D_pai == 0 and self.exit_game_flag == 0:
                ...
                if self.run_time - self.last_time  > 120 :
                    self.last_time = self.run_time


                    while (self.wuqi_process() == 1 ):
                        1
                    if self.yijianronghe_judge() or self.run_time < 600:
                        self.yijianronghe()
                        self.yijianronghe_cnt = self.yijianronghe_cnt + 1
                        while (self.wuqi_process() == 1):
                            1
                    if self.yijianronghe_cnt % 8 == 0:
                        self.share_zhuangbei()
                ...
                if self.start_D_pai == 1:
                    if (self.battle_phase == 1 and self.renkou_cur >= 2) \
                            or (
                            self.battle_phase == 1 and self.refresh_cnt_max[self.battle_phase - 1] == self.refresh_cnt):
                        self.cur_phase_deinit()  # 清零刷新次数, 停止D牌, 隐藏购买界面
                    if (self.battle_phase == 2 and self.renkou_cur >= 3) \
                            or (self.battle_phase == 2 and self.refresh_cnt_max[
                        self.battle_phase - 1] == self.refresh_cnt):  # 2
                        self.cur_phase_deinit()  # 清零刷新次数, 停止D牌, 隐藏购买界面

                    if (self.battle_phase == 3 and self.renkou_cur >= 4) \
                            or (self.battle_phase == 3 and self.refresh_cnt_max[
                        self.battle_phase - 1] == self.refresh_cnt):  # 3
                        self.cur_phase_deinit()  # 清零刷新次数, 停止D牌, 隐藏购买界面

                    if (self.battle_phase == 4 and self.renkou_cur >= 6) \
                            or (self.battle_phase == 4 and self.refresh_cnt_max[
                        self.battle_phase - 1] == self.refresh_cnt):  # 4
                        self.cur_phase_deinit()  # 清零刷新次数, 停止D牌, 隐藏购买界面
                        self.update_hun()

                    if self.battle_phase == 5 and self.renkou_cur >= 8 \
                            or (self.battle_phase == 5 and self.refresh_cnt_max[
                        self.battle_phase - 1] == self.refresh_cnt):  # 5阶段一起 9人口
                        self.cur_phase_deinit()  # 清零刷新次数, 停止D牌, 隐藏购买界面
                        self.update_hun(5)

                    if self.battle_phase == 6 and self.renkou_cur >= 10 \
                            or (self.battle_phase == 6 and self.refresh_cnt_max[
                        self.battle_phase - 1] == self.refresh_cnt):  # 6阶段一起 11人口
                        self.cur_phase_deinit()  # 清零刷新次数, 停止D牌, 隐藏购买界面
                        self.update_hun(5)

                    if self.battle_phase == 7 and self.renkou_cur >= 12 \
                            or (self.battle_phase == 7 and self.refresh_cnt_max[
                        self.battle_phase - 1] == self.refresh_cnt):  # 7阶段 13人口
                        self.cur_phase_deinit()  # 清零刷新次数, 停止D牌, 隐藏购买界面
                        self.update_hun(3)

                    if self.battle_phase == 8 and self.renkou_cur >= 14 \
                            or (self.battle_phase == 8 and self.refresh_cnt_max[
                        self.battle_phase - 1] == self.refresh_cnt):  # 8阶段 15
                        self.cur_phase_deinit()  # 清零刷新次数, 停止D牌, 隐藏购买界面
                        self.update_hun(2)

                    if self.battle_phase == 9 and self.renkou_cur >= 15 :
                        self.cur_phase_deinit()  # 清零刷新次数, 停止D牌, 隐藏购买界面
                ...
                if self.check("shuaxin", 1700, 1920, 800, 1080) and self.start_D_pai == 1:  ## 刷新牌逻辑 gzz3
                    ...
                    if self.wujiang_recognize_cnt == 3 :
                        ...
                        try:
                            ...
                            if self.name_readyToWork == 'NA':  # 这3个都不在白名单里
                                # 做点啥呢
                               # self.check('duiyouxuyao', 450,1250,0,540)
                                self.share_wujiang()
                                continue
                            ...
                            while buy_max > 1:
                                ...
                                if self.check_wujiang_bottom(name):
                                    ...
                                    buy_max = buy_max - 1
                            ########## 这里买武将
                            if self.buy_cnt[name] == 0 and self.check_more('shengji', 2):
                                self.click(0.5)
                                self.get_screen()
                                log.info("很奇怪 name:%s",name)
                            ...
                            #买好了的武将标记下
                            if  name in self.lv1_list_raw:
                                self.lv1_already_have.append(self.name_readyToWork)
